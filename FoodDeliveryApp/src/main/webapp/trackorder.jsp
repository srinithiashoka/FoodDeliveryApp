...
<h1 style="text-align:center; color:#ff4757; margin-top:30px;">Track Your Order</h1>
...
<% if(order != null) { %>
    ...
    <div class="order-row"><span>Total Amount:</span><span>₹<%= String.format("%.2f", order.getTotalAmount()) %></span></div>
    <div class="order-row"><span>Payment Mode:</span><span><%= order.getPaymentMode() %></span></div>
    <div class="order-row"><span>Status:</span><span style="color:<%= statusColor %>; font-weight:bold;"><%= order.getStatus() %></span></div>
